...
public class TC_21_VerifyPointsConversionKohlerRewardCardPageTest extends BaseClass{
	...
	public void testRun() throws EncryptedDocumentException, IOException, InterruptedException {
		...
		//-----------------------------------------------------------------------------------------
			 ...
			 List<WebElement> element = driver.findElements(By.xpath("(//td[3])"));
			 Iterator it=element.iterator();
			 int i=1;
			while (it.hasNext()) {
				it.next();
				String text = driver.findElement(By.xpath("(//td[3])["+i+"]")).getText();
				if (userid.equals(text)) {
					Thread.sleep(5000);
					driver.findElement(By.xpath("(//td[3])["+i+"]/..//label[contains(@class,'mat-checkbox-layout')]")).click();
					Thread.sleep(3000);
					driver.findElement(By.xpath("((//td[3])["+i+"]/..//div/div)[1]")).click();
